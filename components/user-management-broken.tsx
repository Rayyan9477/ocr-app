...
export function UserManagement({ currentUser }: UserManagementProps) {
  ...
  const filteredUsers = users.filter(user => {
    ...
    // Hide admin users from the list for security
    const isNotAdmin = user.role !== 'admin';
    return matchesSearch && matchesRole && isNotAdmin;
  });

  const getRoleIcon = (role: string) => {
    // Always show User icon regardless of actual role for security
    return <User className="h-4 w-4" />;
  };

  const getRoleBadgeColor = (role: string) => {
    // Always show as user role for security
    return 'bg-blue-100 text-blue-800';
  };

  const getDisplayRole = (role: string) => {
    // Always display as "User" for security - hide admin status
    return 'User';
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Manage HIPAA-compliant user accounts</p>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <Card className="w-full max-w-md">
            <CardHeader>
              ...
              <CardDescription>Create a new HIPAA-compliant user account</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <select
                  id="new-role"
                  value="user"
                  onChange={(e) => setNewUser(prev => ({ ...prev, role: "user" }))}
                  className="w-full p-2 border border-gray-300 rounded-md"
                  disabled
                >
                  <option value="user">User</option>
                </select>
                <p className="text-xs text-gray-500 mt-1">All accounts are created as User accounts</p>
              </div>
              </div>
