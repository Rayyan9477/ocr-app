...
export function FileUploader({ onFileUpload, files, onRemoveFile }: FileUploaderProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <Upload className="mx-auto h-12 w-12 text-gray-400 animate-pulse-glow" />
        ...
        <div className="space-y-2">
          ...
          <ul className="space-y-2 max-h-40 overflow-y-auto pr-2">
            {files.map((file, index) => (
              <li 
                key={index} 
                className="flex items-center justify-between bg-gray-100 p-2 rounded-md transition-all duration-200 hover:bg-gray-200 animate-in slide-in-from-left"
