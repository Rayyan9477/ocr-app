...
try {
  ...
  Paligemma2VLService = module.Paligemma2VLService; // Using the named export directly
} catch (error) {
  console.warn('PaliGemma2 service not available, trying compatibility layer');
  try {
    const compatModule = await import('./paligemma2-vlm-service-compat.js');
    Paligemma2VLService = compatModule.Paligemma2VLService;
    console.log('Using PaliGemma2 compatibility layer');
  } catch (compatError) {
    console.warn('PaliGemma2 compatibility layer not available, using simple version');
  }
}
...
const MODELS = {
  paligemma2: {
    id: 'paligemma2-3b-mix-224',  // Updated to match expected model ID
    fallbackIds: [
      'onnx-community/paligemma2-3b-pt-224',
      'google/paligemma2-3b-pt-224',
      'NSTiwari/paligemma2-3b-mix-224-onnx',
      'paligemma2-3b-mix-224'  // Added this as an additional fallback ID
    ],
...
class VLMModelManager {
    ...
    try {
      ...
      if (Paligemma2VLService && !this.paligemma2Service) {
        ...
        try {
          ...
          const modelPath = this.options.modelPaths && Array.isArray(this.options.modelPaths) && this.options.modelPaths.length > 0 
      ...
      const errorMsg = `All PaliGemma2 initialization strategies failed after ${attempts + 1} attempts`;
      console.error(`❌ ${errorMsg}`);
      
      // If we still have retries left, throw a recoverable error
      if (attempts < maxAttempts - 1) {
        throw new Error(errorMsg);
      }
      
      // If we've exhausted retries, fall back to basic functionality
      console.warn('⚠️ Falling back to basic OCR functionality - PaliGemma2 disabled');
      return this.paligemma2 || null;
    ...
    try {
      ...
      if (this.enhancedPaligemma2 && model === this.enhancedPaligemma2) {
        ...
        if (mode === 'text_extraction') {
          result = await this.enhancedPaligemma2.extractText(processablePath, {
            preserveLayout: true,
            enhanceQuality: true
          });
        } else if (mode === 'document_analysis') {
          result = await this.enhancedPaligemma2.analyzeDocument(processablePath);
        } else {
          // Default to assist mode
          result = await this.enhancedPaligemma2.processWithPrompt(processablePath, prompt);
        }
        
        return {
          text: result.text || result.extractedText || result.content || '',
          confidence: result.confidence || result.confidenceScore || 0.85,
          model: 'paligemma2-enhanced',
          modelUsed: 'PaliGemma2 Enhanced Integration'
        };
      }
      
      // Use PaliGemma2 service if available
      if (this.paligemma2Service && model === this.paligemma2Service) {
        console.log('🔧 Using PaliGemma2 service');
        
        // Get the model status to understand capabilities
        const modelStatus = this.paligemma2Service.getStatus ? 
                            this.paligemma2Service.getStatus() : 
                            { processorOnly: false };
        
        // Process the image
        const serviceResult = await this.paligemma2Service.processImage(processablePath, {
          prompt: prompt
        });
        
        return {
          text: serviceResult.text || '',
          confidence: serviceResult.confidence || 0.8,
          model: 'paligemma2-service',
          processingTime: serviceResult.processingTime,
          status: modelStatus.processorOnly ? 'limited' : 'full',
          modelUsed: modelStatus.processorOnly ? 
                    'PaliGemma2 Service (Processor Only)' : 
                    'PaliGemma2 Service'
        };
      }
      
      // Use simple PaliGemma2 if available
      if (this.paligemma2 && model === this.paligemma2) {
        console.log('⚡ Using simple PaliGemma2');
        
        const simpleResult = await this.paligemma2.processImage(processablePath, prompt);
        
        return {
          text: simpleResult.text || '',
          confidence: simpleResult.confidence || 0.7,
          model: 'paligemma2-simple',
          status: simpleResult.status || 'unknown',
          modelUsed: simpleResult.modelType || 'PaliGemma2 Simple'
        };
      }
      
      throw new Error('No PaliGemma2 implementation could process the image');
    ...
    try {
      ...
      if (Buffer.isBuffer(data)) {
        ...
        if (typeof data.arrayBuffer === 'function') {
          ...
          throw new Error('Data object format not supported');
        }
      } else {
  ...
  getModelStatus() {
    ...
    for (const [key, config] of Object.entries(MODELS)) {
      status[key] = {
        ...
        health: 'healthy',
        hasProcessor: true,
        hasModel: key === 'paligemma2' ? false : true  // PaliGemma2 is in processor-only mode
      };
    }
  ...
  async checkModelHealth(modelKey) {
    ...
    return {
      ...
      loaded: true,
      hasProcessor: true,
      hasModel: modelKey === 'paligemma2' ? false : true // PaliGemma2 is in processor-only mode
    };
  }
  ...
  async checkAllModels() {
    ...
    for (const modelKey of Object.keys(MODELS)) {
      try {
        statuses[modelKey] = await this.checkModelHealth(modelKey);
      } catch (error) {
        console.error(`Error checking ${modelKey} health:`, error);
        statuses[modelKey] = {
          health: 'error',
