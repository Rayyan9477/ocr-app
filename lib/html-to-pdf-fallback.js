...
async function convertHtmlToPdf(htmlPath, pdfPath, title = 'OCR Result') {
  try {
    ...
    try {
      ...
      console.log('Using wkhtmltopdf for conversion');
      await execAsync(`wkhtmltopdf "${htmlPath}" "${pdfPath}"`);
      return true;
    } catch (error) {
      console.log('wkhtmltopdf not available or failed, trying next method');
    }
    ...
    if (fs.existsSync(fallbackScript)) {
      try {
        console.log('Using shell script fallback for conversion');
        await execAsync(`bash "${fallbackScript}" "${pdfPath}" "${title}" "${htmlPath}"`);
        if (fs.existsSync(pdfPath)) {
          return true;
        }
      } catch (error) {
        console.log('Shell script fallback failed, trying next method');
      }
    }
